...
public class SelectInfo {
   ...
   public static Todo SelectOne(String id, int num) throws SQLException, UnsupportedEncodingException {
      Connection con = null;
      PreparedStatement pstmt = null;
      ResultSet rset = null;
      Todo todos = null;
      try {
         con = DBUtil.getConnection();
         pstmt = con.prepareStatement("SELECT * FROM TODO WHERE USRID=? AND ROWNUM=?");
         pstmt.setString(1, id);
         pstmt.setInt(2, num);
         rset = pstmt.executeQuery();
         
         while(rset.next()) {
            todos = new Todo(rset.getString(1), rset.getString(2), rset.getDate(3), rset.getDate(4), rset.getString(5), rset.getInt(6), rset.getInt(7));
         }
